...
int main (int argc, char * argv[])
{
    ...
    const char *s2_qname  = argv[1];
    const char *rsp_qname = argv[2];

    // Opening  queues
    mqd_t s2_mq = mq_open(s2_qname, O_RDONLY);
    if (s2_mq == (mqd_t)-1) {
    ...
    while (1) {
        ...
        while (mq_receive(s2_mq, (char *)&msg, sizeof(ipc_msg_t), NULL) == -1) {
            if (errno == EINTR) continue;
            perror("worker_s2: mq_receive(S2)");
            mq_close(rsp_mq);
            mq_close(s2_mq);
            return 1;
        }
    ...
    if (mq_close(rsp_mq) == -1) {
        perror("worker_s2: mq_close(Rsp)");
    }
    if (mq_close(s2_mq) == -1) {
        perror("worker_s2: mq_close(S2)");
    }
